...
public class AES {
        ...
        // Helper method to print integer arrays as hex strings
        private static void printByteArray(int[] array) {
        ...
        public static int[] encrypt(String plaintext, int[][] keySchedule) {
                ...
                for (int paddingIndex = length; paddingIndex < input.length; paddingIndex++) {
        ...
        public static String decrypt(int[] ciphertext, int[][] keySchedule) {
                ...
                int paddingIndex = ciphertext.length - 1;
                while (paddingIndex >= 0 && ciphertext[paddingIndex] == (byte) 0x00) {
                        paddingIndex--;
                }

                int unpaddedLength = paddingIndex + 1;
                int[] unpaddedText;
                if (unpaddedLength >= 0 && unpaddedLength <= ciphertext.length) {
                        unpaddedText = new int[unpaddedLength];
                        System.arraycopy(ciphertext, 0, unpaddedText, 0, unpaddedLength);
                } else {
                        throw new IllegalArgumentException("Invalid padding value");
                }

                StringBuilder plaintext = new StringBuilder();
                for (int value : unpaddedText) {
        ...
        public static int[][] keyExpansion(int[] key) {
                ...
                for (int currentKeyIndex = 0; currentKeyIndex < nKeys; currentKeyIndex++) {
                        int[] row = { key[4 * currentKeyIndex], key[4 * currentKeyIndex + 1],
                                        key[4 * currentKeyIndex + 2],
                                        key[4 * currentKeyIndex + 3] };
                ...
                for (int currentKeyIndex = nKeys; currentKeyIndex < (nBytes * (nRounds + 1)); currentKeyIndex++) {
                        ...
                        // XOR-ing for the next round
                        for (int currentColIndex = 0; currentColIndex < 4; currentColIndex++) {
                                keySchedule[currentKeyIndex][currentColIndex] = keySchedule[currentKeyIndex
                                                - nKeys][currentColIndex]
                                                ^ temp[currentColIndex];
                        }
                }
        ...
        public static int[] rotWord(int[] word) {
                int[] rotatedWord = new int[word.length];
        ...
        private static int[][] mixColumnsInverse(int[][] state, int nBytes) {
                ...
                for (int col = 0; col < 4; col++) {
                        ...
                        state[0][col] = multiply(0x0e, s0) ^ multiply(0x0b, s1) ^ multiply(0x0d, s2)
                                        ^ multiply(0x09, s3);
                        state[1][col] = multiply(0x09, s0) ^ multiply(0x0e, s1) ^ multiply(0x0b, s2)
                                        ^ multiply(0x0d, s3);
                        state[2][col] = multiply(0x0d, s0) ^ multiply(0x09, s1) ^ multiply(0x0e, s2)
                                        ^ multiply(0x0b, s3);
                        state[3][col] = multiply(0x0b, s0) ^ multiply(0x0d, s1) ^ multiply(0x09, s2)
                                        ^ multiply(0x0e, s3);
                }
